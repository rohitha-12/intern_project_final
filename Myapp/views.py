...
def room(request, room_name):  # 👈 Add room_name here
...
class LinkedInAuthView(View):
    ...
    def get(self, request):
        ...
        # Build LinkedIn authorization URL with correct scopes
        auth_params = {
            'response_type': 'code',
            'client_id': LINKEDIN_CLIENT_ID,
            'redirect_uri': LINKEDIN_REDIRECT_URI,
        ...
        # Build URL properly
        auth_url = f"{LINKEDIN_AUTH_URL}?" + "&".join([f"{k}={requests.utils.quote(str(v))}" for k, v in auth_params.items()])
        return redirect(auth_url)
...
class LinkedInCallbackView(View):
    ...
    def get(self, request):
        ...
        if error:
            ...
            return redirect(f"{redirect_url}?linkedin_error={requests.utils.quote(error_description)}")
        ...
        try:
            ...
            if 'error' in token_data:
                ...
                return redirect(f"{redirect_url}?linkedin_error={requests.utils.quote(error_msg)}")
            ...
            return redirect(f"{redirect_url}?linkedin_error={requests.utils.quote(f'Authentication failed: {str(e)}')}")
    ...
    def get_access_token(self, code):
        ...
        try:
            ...
            return {'error': 'token_request_failed', 'error_description': str(e)}
    ...
    def get_linkedin_profile(self, access_token):
        ...
        try:
            ...
            raise Exception(f"Profile request failed: {str(e)}")
    ...
    def generate_linkedin_token(self, profile_data, email):
        ...
        # Extract profile information - updated for new API format
        linkedin_id = profile_data.get('id')
        
        # Handle the new LinkedIn API response format
        first_name = ''
        last_name = ''
        
        if 'firstName' in profile_data:
            first_name_data = profile_data['firstName']
            if 'localized' in first_name_data:
                # Get the first available localized name
                localized_names = first_name_data['localized']
                if localized_names:
                    first_name = list(localized_names.values())[0]
        
        if 'lastName' in profile_data:
            last_name_data = profile_data['lastName']
            if 'localized' in last_name_data:
                # Get the first available localized name
                localized_names = last_name_data['localized']
                if localized_names:
                    last_name = list(localized_names.values())[0]
